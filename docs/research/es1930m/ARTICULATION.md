...
`scripts/validate_es1930m_kinematics.py` samples 101 lift states, checks both crossing links in every level, shared-center closure, the fixed rear anchor, branch monotonicity, transform continuity, tolerance-bounded presentation-cylinder stroke, and collision/envelope proxies. `scripts/validate_es1930m_glb.py` separately checks the exported node hierarchy, explicit marker/link counts, asset envelope, interaction volumes, and hash-bound runtime contract. The browser then poses the actual GLB through stowed, intermediate, and raised states and checks rear anchors, both slide pairs, cylinder attachment, and moving hit volumes. None is a substitute for physical-machine validation.
