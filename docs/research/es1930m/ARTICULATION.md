...
### Frozen connected reconstruction H2

PVC 2404 service Figures 58 and 59 establish the lower block/pin arrangement and label the machine front; Figure 62 separates the fixed platform pin pair from the opposite upper-slide-block pair. The removal sentence on p. 197 calls the lower channel "front" while the immediately following installation sentence calls it "rear." Because that wording is internally inconsistent, the directional branch is reconciled from the three illustrated relationships together with the current standard-machine raised side view: the front boundary is fixed and the paired lower and upper blocks travel at the rear, opposite the platform-extension end. Pages 203-205 and 217-221 establish a lower cylinder pin, upper cylinder pin, kicker pivot and separate kicker roller pin.

The browser therefore keeps the front boundary fixed, places both lower and upper blocks at their rear-track coordinates, and derives every boundary from a single five-level pantograph state. The platform extension retains its documented frontward travel. The cylinder upper pin and three-point kicker presentation are mirrored onto Level01 branch B; cylinder pin distance emerges from that posed graph. H2 attachment fractions are calibrated only so the endpoint distance change falls within the published 685.5 +/- 1.5 mm stroke. They are visual reconstruction coordinates, not JLG fabrication data or a claimed factory leverage curve.
...
The frozen standard platform uses a foot-actuated extension with published 0.55 m travel. Parts Figure 4-3 establishes two front rollers, two rear rollers, bumpers, pads, supports, spring/rod/bracket and platform-extension weldment `1001324255`. Parts Figure 4-7 separates the full-length fixed outer side rails from the front-extension rail assembly, while the service cable route identifies adjacent platform and extension vertical-rail branches. The authored moving rails and toe boards are therefore nested inboard of the fixed outer rails, retain at least 18 mm of reconstructed longitudinal overlap at full travel, and preserve at least 3 mm of authored lateral mesh clearance between guard solids. Those offsets and overlaps are presentation coordinates constrained to the published envelope, not measured tube sections or fabrication clearances. Both the exported GLB validator and posed browser self-test measure the actual mesh bounds and enforce the individual fixed/moving parent edges at stowed, midpoint and full travel.
...
The cylinder installation uses connected H1 link-frame coordinates. A compact off-center triangle carries distinct scissor-pivot, cylinder-pin and roller axes; its exact local offsets remain reconstruction. Cylinder pin distance emerges from the pose and its endpoint change is constrained to the published 0.6855 m stroke. Roller contact geometry remains unresolved and is not claimed as the factory leverage curve.

`scripts/validate_es1930m_kinematics.py` samples 101 lift states, checks both crossing links in every level, shared-center closure, the fixed front anchor, branch monotonicity, transform continuity, tolerance-bounded presentation-cylinder stroke, and collision/envelope proxies. `scripts/validate_es1930m_glb.py` separately checks the exported node hierarchy, explicit marker/link counts, asset envelope, interaction volumes, and hash-bound runtime contract. The browser then poses the actual GLB through stowed, intermediate, and raised states and checks front anchors, both rear slide pairs, cylinder attachment, and moving hit volumes. None is a substitute for physical-machine validation.
