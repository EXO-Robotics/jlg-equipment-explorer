...
export function solve742RigGeometry(input,authored={midX:.12,flyX:.12}){const state=input?.boomAngle===undefined?solve742State(input):input,beams={},points={},cylinders={},pivot=[-2.158,1.838,0];
  ...
  [-.18,-.10,.10,.18].forEach((lateral,lane)=>{const start=[.15,-.12,lateral],end=[movingHoseEndX,-.40,lateral];addHoseULoop(beams,`BoomHose_${lane}`,start,end,JLG742_MECHANISM.boomHoseTotalLength,JLG742_MECHANISM.boomHoseLoopRadius);});
  ...
  const carriagePivot=[5.296,-.8,0],tiltBase=JLG742_MECHANISM.carriageTiltBase,tiltAnchor=add(carriagePivot,rotateBoom([-.18,.23002765448666934,JLG742_MECHANISM.carriageTiltLateral],state.carriageAngle)),tiltLinkAnchor=add(carriagePivot,rotateBoom([-.08,.45,JLG742_MECHANISM.carriageTiltLateral],state.carriageAngle));cylinders.carriageTilt=fixedCylinder(beams,"CarriageTiltCylinderBarrel","CarriageTiltCylinderRod",tiltBase,tiltAnchor,JLG742_MECHANISM.barrelLengths.carriageTilt);beams.CarriageTiltLink=[tiltAnchor,tiltLinkAnchor];points.CarriageTiltCylinderRodPin=tiltAnchor;points.CarriageTiltLinkPin=tiltLinkAnchor;
