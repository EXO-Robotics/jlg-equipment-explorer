...
for(let index=0;index<=2000;index+=1){
  ...
  for(let lane=0;lane<4;lane+=1){const segments=hoseNames(`BoomHose_${lane}`).map(name=>geometry.beams[name]);
    for(const segment of segments)for(let sample=0;sample<=20;sample+=1){const amount=sample/20,point=segment[0].map((value,axis)=>value+(segment[1][axis]-value)*amount);for(const tube of rigidTubes)minimumBoomHoseTubeClearance=Math.min(minimumBoomHoseTubeClearance,distanceToSegment(point,tube)-.025);}
    for(let index=2;index<segments.length-1;index+=1){const before=sub(segments[index-1][1],segments[index-1][0]),after=sub(segments[index][1],segments[index][0]),cosine=Math.max(-1,Math.min(1,dot(before,after)/(Math.hypot(...before)*Math.hypot(...after))));maximumBoomHoseDirectionChange=Math.max(maximumBoomHoseDirectionChange,Math.acos(cosine));}
  }
...
if(maxFrontModeRelativeError>.05)throw new Error(`front-mode ICR relative spread ${maxFrontModeRelativeError} exceeds reconstructed limit`);
