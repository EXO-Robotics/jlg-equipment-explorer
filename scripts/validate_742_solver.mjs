...
const hoseNames=(prefix)=>Array.from({length:3},(_,index)=>`${prefix}_${index}`);
...
let samples=0,minForkY=Infinity,maxForkY=-Infinity,maxHeight=-Infinity,maxCenterSpread=0,maxCenterRelativeSpread=0,maxAckermannFitError=0,maxAckermannRelativeError=0,maxFrontModeRelativeError=0;
...
let maximumHoseEndpointStep=0,priorHoseEndpoints=null;
...
for(let index=0;index<=2000;index+=1){
  ...
  if(index>0){
    ...
    maxFrontModeRelativeError=Math.max(maxFrontModeRelativeError,frontSpread/Math.max(...frontCenters.map(Math.abs)));
  }
}
