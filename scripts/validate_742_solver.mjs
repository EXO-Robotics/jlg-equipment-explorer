...
let samples=0,minForkY=Infinity,maxForkY=-Infinity,maxHeight=-Infinity,maxCenterSpread=0,maxAckermannFitError=0,maxAckermannRelativeError=0;
...
for(let index=0;index<=2000;index+=1){
  ...
  if(index>0){const state=solve742State({lift:0,telescope:0,tilt:0,steer:index/2000,level:0,steerMode:"circle"}),outer=Math.abs(state.wheelAngles.FL),inner=Math.abs(state.wheelAngles.FR),fromInner=JLG742_MECHANISM.wheelCenterTrack/2+(JLG742_MECHANISM.wheelbase/2)/Math.tan(inner),fromOuter=(JLG742_MECHANISM.wheelbase/2)/Math.tan(outer)-JLG742_MECHANISM.wheelCenterTrack/2,error=Math.abs(fromInner-fromOuter);maxAckermannFitError=Math.max(maxAckermannFitError,error);maxAckermannRelativeError=Math.max(maxAckermannRelativeError,error/Math.max(fromInner,fromOuter));}
}
...
for(let li=0;li<liftValues.length;li+=1)for(let ti=0;ti<telescopeValues.length;ti+=1){
  ...
  for(const tilt of tiltValues)for(const level of levelValues)for(const mode of modes)for(const steer of steerValues){
    ...
    if(mode==="circle"&&Math.abs(steer)>1e-9){const front=steer>0?state.wheelAngles.FR:state.wheelAngles.FL,rear=steer>0?state.wheelAngles.RL:state.wheelAngles.RR,frontCenter=JLG742_MECHANISM.wheelCenterTrack/2+(JLG742_MECHANISM.wheelbase/2)/Math.tan(Math.abs(front)),rearCenter=JLG742_MECHANISM.wheelCenterTrack/2+(JLG742_MECHANISM.wheelbase/2)/Math.tan(Math.abs(rear));maxCenterSpread=Math.max(maxCenterSpread,Math.abs(frontCenter-rearCenter));}
  }
}
